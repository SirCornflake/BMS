                   ...
                   beta.ini=base::rep(1,length(Covariates[1,]) +1), invsigma2.ini=1, w.ini=base::rep(1,length(y)), count.iteration=TRUE )
{
  ...
  list(beta_chain=beta.chain, sigma2_chain=sigma2.chain, model_chain=model_chain, Regression="QR", prednames=prednames, Seconds=Time)
}
...
summary_gibbs<-function(fit)
{
  ...
  rowbase::names(Table)<-c(aux_rownames, "intercept", fit$prednames)
